...
{
    public async Task<string> StructuredAsync(string instructions, object input, object schema, CancellationToken token)
    {
        ...
        request.Content = JsonContent.Create(new {
            ...
            max_completion_tokens = 3000, store = false,
            response_format = new { type = "json_schema", json_schema = new { name = "prestart", strict = true, schema } },
            messages = new[] {
                new { role = "system", content = instructions },
                new { role = "user", content = JsonSerializer.Serialize(input, new JsonSerializerOptions(JsonSerializerDefaults.Web)) }
            }
        });
        ...
        if (message.TryGetProperty("refusal", out var refusal) && refusal.ValueKind == JsonValueKind.String)
            throw new HttpRequestException("Form answer unavailable.");
        var reply = message.GetProperty("content").GetString()?.Trim();
        if (string.IsNullOrEmpty(reply) || reply.Length > 16000) throw new HttpRequestException("Invalid form answer.");
        logger.LogInformation("Pre-start structured model completed in {ElapsedMs}ms", timer.ElapsedMilliseconds);
        return reply;
    }
