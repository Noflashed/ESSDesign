...
try {
 ...
 assert.equal(await page.getByRole('option',{name:'All Builders',exact:true}).count(),1);
 ...
 assert.equal(await page.getByRole('option',{name:'All Projects',exact:true}).count(),1);
 ...
 const dateFields = page.getByText('DATE:', {exact:true});
 const initialDate = (await dateFields.nth(0).locator('..').innerText()).match(/(\d{2})\/(\d{2})\/(\d{4})/);
 assert.ok(initialDate);
 const selectedDate = `23/${initialDate[2]}/${initialDate[3]}`;
 const weekday = day => new Intl.DateTimeFormat('en-AU',{weekday:'long',timeZone:'UTC'}).format(new Date(`${initialDate[3]}-${initialDate[2]}-${day}T12:00:00Z`));
 const selectedDisplay = `${weekday('23')} ${selectedDate}`;
 await dateFields.nth(0).locator('..').getByRole('button').click();
 await page.getByText('23', {exact:true}).click();
 assert.ok((await dateFields.nth(0).locator('..').innerText()).includes(selectedDisplay));
 await dateFields.nth(1).locator('..').getByRole('button').click();
 await page.getByText('23', {exact:true}).click();
 assert.ok((await dateFields.nth(1).locator('..').innerText()).includes(selectedDisplay));
 ...
 const overtime = page.getByRole('checkbox',{name:'Overtime for labour row 1',exact:true});
 assert.equal(await overtime.getAttribute('aria-checked'),'false');
 await overtime.click();
 assert.equal(await field('TOTAL:').inputValue(),'24 OT');
 await field('HOURS:').fill('10');
 assert.equal(await field('TOTAL:').inputValue(),'30 OT');
 await field('HOURS:').fill('8');
 await overtime.click();
 assert.equal(await field('TOTAL:').inputValue(),'24');
 await overtime.click();
 ...
 await checkFormZoom();
 await page.screenshot({path:'/tmp/ess-day-labour-overtime-form.png'});
 ...
 const canvas = await page.getByText('Sign here',{exact:true}).locator('..').elementHandle();
 await canvas.click({position:{x:80,y:70}}); // Wait for the signature modal to settle before drawing.
 ...
 assert.ok(saved.essRepresentativeSignatureStrokes.some(stroke=>stroke.length>2));
 ...
 await page.getByRole('button',{name:'Builder',exact:true}).click();
 await page.getByRole('option',{name:'Other Builder',exact:true}).click();
 await page.getByRole('button',{name:'Project',exact:true}).click();
 await page.getByRole('option',{name:'Second Site',exact:true}).click();
 await page.reload();
 await page.getByRole('button',{name:'Add Day Labour',exact:true}).waitFor();
 assert.match(await page.getByRole('button',{name:'Builder',exact:true}).innerText(), /Other Builder/);
 assert.match(await page.getByRole('button',{name:'Project',exact:true}).innerText(), /Second Site/);
 await page.evaluate(() => localStorage.setItem('ess-day-labour-selection-v1:user-test', JSON.stringify({builderId:'builder-other',projectId:'removed-site'})));
 await page.reload();
 await page.getByRole('button',{name:'Add Day Labour',exact:true}).waitFor();
 assert.match(await page.getByRole('button',{name:'Project',exact:true}).innerText(), /First Site/);
 await page.evaluate(() => localStorage.setItem('ess-day-labour-selection-v1:user-test', JSON.stringify({builderId:'removed-builder',projectId:'removed-site'})));
 await page.reload();
 await page.getByRole('button',{name:'Add Day Labour',exact:true}).waitFor();
 assert.match(await page.getByRole('button',{name:'Builder',exact:true}).innerText(), /Test Builder/);
 assert.match(await page.getByRole('button',{name:'Project',exact:true}).innerText(), /Test Project/);
