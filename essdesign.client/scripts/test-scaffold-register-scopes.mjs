...
const visible = async names => {
 ...
 await page.waitForFunction(expected => {
  const names = [...document.querySelectorAll('.scaffold-card-name h2')].map(el => el.textContent).sort();
...
try {
 ...
 assert.equal(await page.locator('.scaffold-register-card').filter({hasText:'First scaffold'}).locator('.scaffold-card-name p').last().innerText(),'Test Project');
 ...
 await visible(['First scaffold','Second scaffold']);
 await choose('Client','All Builders');
 await visible(['First scaffold','Second scaffold','Other scaffold']);
 assert.deepEqual(await page.locator('.scaffold-register-card').filter({hasText:'Other scaffold'}).locator('.scaffold-card-name p').allTextContents(),['Other Builder','Test Project']);
 ...
 await page.reload();
 await visible(['First scaffold','Second scaffold','Other scaffold']);
 await page.locator('.scaffold-register-card').filter({hasText:'Other scaffold'}).click({button:'right'});
 ...
 await choose('Client','Test Builder');
 await visible(['First scaffold','Second scaffold']);
