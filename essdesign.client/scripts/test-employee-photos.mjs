...
try {
    ...
    const photoButton = page.getByRole('button', { name: 'Change employee profile photo' });
    const overlay = photoButton.locator('.employee-profile-photo-overlay');
    ...
    assert.equal(await overlay.evaluate(el => getComputedStyle(el).opacity), '1', 'Keyboard focus exposes the photo action');
    const input = page.getByLabel('Upload employee profile photo');
