...
try {
    for (const width of [1440, 1024, 390]) {
        ...
        const actions = page.locator('.scaffold-register-card').first().locator('.scaffold-register-add-cell');
        ...
        for (const action of await actions.all()) {
            ...
            assert.ok(dimensions.rightGap >= 0);
            await action.click({ trial: true });
        }
        await page.locator('.scaffold-register-cards-wrap').evaluate(el => { el.scrollLeft = el.scrollWidth; el.scrollTop = el.scrollHeight; });
        await checkAdd();
        await page.getByRole('searchbox').fill('no matching scaffold');
        await page.getByText('No scaffolds match the current search or status.').waitFor();
        await checkAdd();
    ...
    console.log('PASS: empty register retains the header Add scaffold action');
} finally {
    await browser.close();
}
