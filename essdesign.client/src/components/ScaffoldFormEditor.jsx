...
export default function ScaffoldFormEditor({screen, params, onClose, onSaved}) {
    ...
    const navigation = useMemo(() => ({
        ...
        navigate(nextScreen, nextParams) {
            ...
            } else if (['HandoverCertificateForm', 'ScaffTagForm'].includes(nextScreen)) {
                setStack(previous => [...previous, {screen: nextScreen, params: nextParams}]);
            } else onClose();
        },
        reset: onClose,
    }), [stack.length, onClose, onSaved]);
    return <div className="scaffold-form-editor" role="dialog" aria-modal="true" aria-label={current.screen === 'ScaffTagForm' ? 'Scaff-Tag form' : 'Handover Certificate form'}>
        {stack.map((entry, index) => {
            const EntryScreen = entry.screen === 'HandoverCertificateForm' ? HandoverCertificateFormScreen : ScaffTagFormScreen;
