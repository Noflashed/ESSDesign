...
const scopeTest = new URLSearchParams(location.search).has('scope');
if (scopeTest) {
 builders[0].projects.push({id:'project-second',name:'Second Site'}, {id:'project-empty',name:'Empty Site'});
 builders.push({id:'builder-other',name:'Other Builder',projects:[{id:'project-test',name:'Test Project'}]});
}
...
for (const [api, type, method] of [[scaffoldRegisterAPI, 'scaffold-register', 'listRecords'], [scaffTagsAPI, 'scaff-tags', 'listForms'], [handoverCertificatesAPI, 'handover-certificates', 'listForms']]) {
 api[method] = async (builderId, projectId) => (await (await fetch('/fixture/forms?type='+type)).json())
  .filter(form => form.builderId === builderId && form.projectId === projectId);
}
...
createRoot(document.getElementById('root')).render(<ScaffoldRegisterPage initialBuilderId={scopeTest ? '' : 'builder-test'} initialProjectId={scopeTest ? '' : 'project-test'} />);
